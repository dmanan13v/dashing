...
class ReturnCruncher:
    def __init__(self, data_dict: dict, marks: Optional[list] = None) -> None:
        self.titre: AssetMulti = getattr(AssetMap(**load_config("asset_map")), data_dict.get("asset"))
    ...
    def load_and_process_data(self, data_dict):
        ...
        tickers = [self.titre.bench, self.titre.bull] + ([self.titre.bear] if self.titre.bear else [])
    ...
    def get_close_to_close(self, ticker: str, marks: list) -> dict[str, Any]:
        ...
        df_filtered = self.data[self.data["ticker"] == ticker].sort_values("Date").reset_index(drop=True)
    ...
    def get_night_return(self, ticker: str, marks: list) -> dict[str, Any]:
        ...
        end = min(end, df_filtered.index.max() - 1)
